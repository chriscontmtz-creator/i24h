...
| `#btn-cerrar-sesion-nav` — `index.hbs:81` (y dropdown de cuenta en `cliente.hbs:44`) | `JAVA.js:375-379` | `POST /api/logout` | `auth.js:77` | — | Destruye sesión, limpia cookie `i24h.sid`. **Corregido 2026-08-10 (BUG-04)** — en `/cliente` el dropdown de cuenta no abría, no se podía cerrar sesión ni navegar: `JAVA.js:159` (`botonPublicar.addEventListener`) corría en top-level sin `?.`, y como el formulario de comentarios (`#submit-comment-btn`) solo existe en el home, en `/cliente` `botonPublicar` era `null` → lanzaba TypeError y **mataba todo el script de ahí en adelante** (dropdown `elNavBtn` línea 374, este logout línea 400, y los canjes línea 425). Se cambió a `botonPublicar?.addEventListener(...)`, consistente con el resto del archivo que ya usaba optional chaining |
...
| Elemento UI (archivo:línea) | Handler JS | Endpoint | Handler de ruta | Modelo | Notas |
|---|---|---|---|---|---|
| Carga automática sección "Usuarios" (admin) / "Mi equipo" (coordinador y líder) | `panel.hbs:2271 usrCargarTodo()` | `GET /api/empleados` | `empleados.js:18` | `Usuario` | **Filtrado por rol** — admin ve `cargo != cliente` completo; un `coordinador` ve `cargo` en `['lider','encargado','colaborador']` cuyas `sucursales` se solapan con las suyas; un `lider` ve solo `cargo:'colaborador'` con sucursal compartida (ambos vía `sucursalesDeUsuario()`, `src/utils/sucursales.js`); cualquier otro cargo recibe 403. **Corregido 2026-08-05** — hasta entonces `coordinador` estaba agrupado con `admin` y veía la lista completa sin filtrar, contradiciendo el propio nombre de la sección "Mi equipo" que ya se le mostraba visualmente en otras partes. Antes de 2026-08-04 era `requireAuth` sin ningún filtro |
...
| Elemento UI (archivo:línea) | Handler JS | Endpoint | Handler de ruta | Modelo | Notas |
|---|---|---|---|---|---|
| Nav sidebar único "Usuarios" (admin) / "Mi equipo" (coordinador, líder) — `panel.hbs:67-73` | navegación de sección (`irSeccion`) | *(sin endpoint propio, dispara la carga de la sección — ver `GET /api/empleados` en sección 3)* | — | — | **Reestructurado 2026-08-05** — antes había DOS botones de nav separados apuntando al mismo `data-seccion="usuarios"`: uno dentro del bloque `{{#if esAdmin}}` (grupo "Sistema", solo admin/coordinador) y otro en un `{{else if esLider}}` aparte (grupo "Equipo", solo líder). Se consolidó en un solo botón, ahora dentro del grupo "Equipo" (mismo lugar donde antes vivía el link muerto a `/empleados.html`, ver "Pendientes de seguridad / calidad conocidos" más abajo — sección "'Empleados' — link muerto y bloque huérfano ELIMINADOS"), visible si `esGestorEquipo` (`vistas.js`: admin/coordinador/líder) con `data-titulo` dinámico según `esAdminPuro` (`vistas.js`: solo `cargo==='admin'`, variable nueva — no confundir con `esAdmin`, que sigue siendo admin+coordinador y sigue gateando Tickets I24H/Promociones/Fotos/Cotizaciones/Configuración sin cambios). La vista que carga sigue recortada para líder (sin tabs de Clientes/Códigos, sin botón "Nuevo empleado", sin menú de acciones por fila) vía `window.USR_ES_LIDER` — coordinador NO tiene ese recorte, conserva el menú completo de acciones (Editar cargo/sucursales/turno/contraseña, Eliminar), solo cambia qué cuentas *ve* en la lista |
...
| Elemento UI (archivo:línea) | Handler JS | Endpoint | Handler de ruta | Notas |
|---|---|---|---|---|
| Pills `.vnt-pill` + `#vnt-suc-filter` — `panel.hbs:562-573` | `vntCargar() panel.hbs:3609` | `GET /api/ventas?sucursal=&periodo=` | `ventas.js:58` | Datos mock (JSON), **no** vienen del sync real; sucursales sin sync conectado (`sucursalConectada()`, `src/utils/sucursales.js`) se devuelven en 0 desde 2026-08-04 en vez de datos de ejemplo. El `<select>` de sucursal usa **slugs** propios (`simon-bolivar`, `SLUG_VENTA` en `vistas.js:16-26`, valor de `sucursalesUsuarioVenta`/`vnt-suc-filter`) distintos del nombre completo con acentos que se usa en todo lo demás (`sucursalPermitida`/`sucursalConectada`/`TODAS_SUCURSALES`) — este endpoint mock sí espera el slug (matchea contra `id` en `ventas.json`), pero ver la fila de abajo para el endpoint real, que NO. Pill de período por defecto **cambiado 2026-08-05** de "Hoy" a "7 días" (`vntPeriodo='7'` en vez de `'hoy'`, `panel.hbs:3564`) porque "Hoy" arranca vacío hasta que hay ventas grandes sincronizadas en el día — afecta a esta tarjeta y a la de abajo, que comparten `vntPeriodo`. **Corregido 2026-08-10 (BUG-03)** — este endpoint tenía solo `requireAuth` y devolvía las 9 sucursales a cualquier empleado logueado (líder/encargado incluidos) cuando `sucursal=todas`, contradiciendo la regla de `sucursales.js` ("admin ve las 9, cualquier otro cargo solo las asignadas") que Dashboard y `/ventas/extraordinarias` sí aplican. Ahora filtra el `desglose` por `sucursalesDeUsuario(req.session.usuario)` (nombre display, coincide con el campo `nombre` de `ventas.json`): admin sigue viendo las 9, líder/encargado/coordinador solo sus asignadas. Verificado: test.lider (Insurgentes) con `sucursal=todas` → solo Insurgentes |
| `#vnt-top-cat-filter` — `panel.hbs:632` | `vntCargarTop() panel.hbs:3733` | `GET /api/ventas/top-productos` | `ventas.js:102` | Igual, filtrado a sucursales conectadas **y a las asignadas al usuario** (BUG-03, 2026-08-10 — antes cualquier empleado veía productos de todas las conectadas) |
| Automático al final de `vntCargar()` | `vntCargarAlertas() panel.hbs:3763` | `GET /api/ventas/alertas` | `ventas.js:121` | También usado por el Dashboard; filtrado a sucursales conectadas **y a las asignadas al usuario** (BUG-03, 2026-08-10) |
...
| Elemento UI (archivo:línea) | Handler JS | Endpoint | Handler de ruta | Modelo | Notas |
|---|---|---|---|---|---|
| Nav "Revisiones" + filtros + tabs — `panel.hbs:42`, `index.hbs:16-60` | `revFiltrar() revisiones.js:97` | `GET /revisiones` | `revisiones.js:105` | `Revision`, `ConteoFisico`, `ContadorImpresora`, `BitacoraTurno`, `CorteCaja`, `Ticket`, `Producto` | Agrega bitácoras + contadores + resumen de caja en 3 tabs. Tab "Resumen de caja" **corregido 2026-08-04**: (1) agrupación de tickets/contadores cambiada de clave `ncaja` a `sucursal+ncaja` (`ncaja` solo es único por sucursal, mezclaba cajas de sucursales distintas al ver "Todas las sucursales"); (2) `.select()` de la consulta de tickets ahora incluye `sucursal` (antes se perdía y la clave compuesta nunca hacía match); (3) INE excluido del cálculo de Diferencia (se cobra pero no tiene contador de impresora); (4) Escáner ahora se suma al total "Diferencia" (antes quedaba aparte); (5) nueva columna "Diferencia" por tipo (BYN/Color copias e impresiones) en vez de solo un total combinado; (6) aviso `rc-brecha-aviso` cuando el corte de una impresora cubre >30h desde su lectura anterior (`ContadorImpresora.delta` = todo ese hueco, no un turno — evita leer una diferencia grande como merma cuando en realidad son varios días acumulados). **Conciliación reescrita 2026-08-11 (Fase 1, regla confirmada por Chris)**: (a) **base de comparación = suma de páginas de TODAS las categorías que consumen contador/hojas**: copias B/N + copias color + **copias INE** + impresión B/N + impresión color (`totalImpreso`). El **INE ya NO se excluye** — imprime físicamente en la copiadora B/N y sí incrementaba el contador, así que antes producía un negativo falso crónico; ahora se suma (también a la fila B/N vía `filaDif(copias_byn+copias_ine, copBYN)`). El **escáner salió del total** (no consume hojas) y se concilia solo en su fila. Tabloide/Doble carta siguen informativos. (b) **Signo invertido**: `diferencia = totalContador − (totalImpreso + merma)`; **positivo = el contador marcó más hojas de las impresas (posible consumo/fuga no cobrado)**, ~0 = cuadra, negativo = sobre-registro (ámbar). Semáforo: `difOk` si `|dif|≤5`, `difMal` (rojo) si `dif>5`, `difWarn` (ámbar) si `dif<−5`. (c) **"Sin lectura" ≠ "contador=0"**: si NO existe `ContadorImpresora` de esa caja (`sinContador = !contPorNcaja[clave]`) **o** la lectura no es confiable (`brechaLarga`: sin corte previo / >30h), `cruceConfiable=false` → `diferencia=null`, `estadoCruce='sin_contador'`/`'contador_no_confiable'`, badge gris "Sin lectura" y **no se calcula descuadre** (elimina los `+130/+300` falsos que venían de tratar la lectura ausente como 0). Verificado con Atlas: Simón Bolívar julio, 94 cortes cerrados → 13 con lectura calculan diferencia, 81 "sin lectura". La vista (`index.hbs`) añade leyenda `rc-leyenda`, gatea badges con `{{#if cruceConfiable}}`, renombra "Total cobradas"→"Total impreso"; `rcGuardarMerma` usa `data-confiable` + fórmula `cnt−(cob+merma)` |
...
### ✅ "Empleados" — link muerto y bloque huérfano ELIMINADOS 2026-08-05

El nav sidebar tenía `<a href="/empleados.html">Empleados</a>` (grupo "Equipo") apuntando a una página estática (`public/empleados.html`) sin backend real detrás — no confundir con el submódulo "staff/auditorías" de `empleados.js` (sigue muerto, ver abajo), es un archivo `.html` aparte. Además existía un bloque `<div class="panel-seccion oculta" id="sec-empleados">` (`panel.hbs`) con el mismo link, que ya era inalcanzable de por sí porque nada tenía `data-seccion="empleados"` para dispararlo. Se borraron los tres: el link del nav, el bloque `#sec-empleados`, y el archivo `public/empleados.html`. En su lugar, el nav ahora tiene un único botón "Usuarios"/"Mi equipo" (ver sección 3, fila del nav sidebar) — no una feature nueva, es la sección "Usuarios" que ya existía, solo que antes competía visualmente con este link muerto en el mismo grupo del sidebar.
...
### ✅ Bugs de UI/datos ARREGLADOS 2026-08-10 (sesión 2 — BUG-01/02/05/06/07/08/09/10)

Todos verificados con `node --check` + compilación Handlebars con contexto simulado (el sandbox no resuelve el SRV de Atlas, así que no hubo repro con DB en vivo).

- **BUG-01 — nav público sin scroll** (`public/JAVA.js`, IIFE del menú hamburguesa): los links `#sec-*` del header marcaban activo pero el salto nativo dejaba el título bajo el nav sticky. Ahora los `a[href^="#"]` del nav hacen `preventDefault` + `scrollTo` con offset del alto del `.nav` (reusa el mecanismo del botón "Iniciar sesión").
- **BUG-02 — sucursales: solo 1 con imagen rota** (`src/routes/vistas.js` `fotosPorSucursal`, `src/views/index.hbs`, `cliente.hbs`, `public/estilos.css`): la sección se alimentaba solo de `FotoSucursal` (solo Simón Bolívar tenía registro) y su archivo no existía en `public/imagenes/sucursales/` (carpeta vacía → `<img>` roto). Ahora `fotosPorSucursal` devuelve SIEMPRE las 9 (`SUCURSALES`), ignora archivos que no existen en disco (`fs.existsSync`), y el template pinta un placeholder `.foto-suc-placeholder` "Foto próximamente" cuando `urls` está vacío.
- **BUG-05 — sucursales cercanas inventadas** (`src/config/constants.js`, `vistas.js`): `SUCURSALES_CLIENTE` tenía Mitras/Santa Catarina/Cumbres (inexistentes). Ahora se deriva de `TODAS_SUCURSALES` (las 9 reales); sin calles/coordenadas inventadas, `mapsUrl` es una búsqueda de Maps por nombre.
- **BUG-06 — contador "9/9 con ventas"** (`src/views/panel.hbs` `vntRenderKpis`): contaba `d.desglose.length` (siempre 9). Ahora filtra `total > 0` → "1 / 9" (solo Simón Bolívar reporta).
- **BUG-07 — "0/9 en línea" vs 9 badges "Abierta"** (`vistas.js` `/panel`, `panel.hbs` sec-sucursales): "Abierta" = negocio abierto 24/7 (las 9, correcto); la métrica ahora es server-render `{{sucursalesEnLinea}}/{{totalSucursales}}` desde `SUCURSALES_CONECTADAS` (1/9) con etiqueta "Con sync en línea", sin contradicción.
...
**BUG-03 — cierre final del scoping por sucursal** (verificación en vivo 9/10; faltaban dos lugares que aún mostraban las 9): mismo criterio que Ventas (`sucursalesDeUsuario`: admin las 9, cualquier otro cargo solo las asignadas).
  - **Vista `/panel → Sucursales`** (`vistas.js` `/panel`, `panel.hbs` sec-sucursales): las 9 tarjetas son HTML estático; se pasa `sucursalesVisibles` (mapa nombre→boolean) y cada tarjeta se envuelve en `{{#if sucursalesVisibles.[Nombre]}}`. Verificado por compilación Handlebars: admin → 9 tarjetas, líder con 1 sucursal → 1, líder con 0 → 0. **Cabecera acotada también** — el contador "Sucursales activas" ya no es `SUCURSALES_CONECTADAS.length / 9` fijo: numerador = de las sucursales visibles del usuario cuántas tienen sync (`sucursalesEnLinea`), denominador = cuántas ve (`totalSucursales = sucursalesUsuario.length`); admin "1 / 9", líder de Simón Bolívar "1 / 1", líder de Insurgentes "0 / 1". La sub "Suma de todas las sucursales" pasó a `recaudacionScope` (admin → "Suma de todas las sucursales", 1 sucursal → su nombre, varias → "Suma de tus sucursales", 0 → "Sin sucursal asignada").
